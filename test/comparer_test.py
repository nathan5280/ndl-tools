...
from ndl_tools.comparer import Comparer
...
def test_compare_dict():
    match = Comparer.compare(TEST_DICT, SORTED_DICT)
    assert match
...
def test_compare_list():
    match = Comparer.compare(TEST_LIST, SORTED_LIST)
    assert match
...
def test_compare_dict_fail():
    ...
    td["l"] = []
    match = Comparer.compare(td, SORTED_DICT)
    assert not match
...
def test_compare_list_fail():
    ...
    tl[0] = []
    match = Comparer.compare(tl, SORTED_LIST)
    assert not match


def test_diff_dict():
    match = Comparer.diff(TEST_DICT, SORTED_DICT)
    assert match
...
def test_diff_dict_fail():
    td = copy.deepcopy(TEST_DICT)
    td["l"] = []
    match = Comparer.diff(td, SORTED_DICT)
    assert not match


def test_diff_list():
    result = Comparer.diff(TEST_LIST, SORTED_LIST)
    assert result
...
def test_diff_list_fail():
    tl = copy.deepcopy(TEST_LIST)
    tl[0] = []
    result = Comparer.diff(tl, SORTED_LIST)
...
def test_json_encoder():
    ...
    result = Comparer.diff(d1, d1, cls=DateJSONEncoder)
    assert result
...
def test_nested_dict_equal():
    d1 = {"a": 1, "b": {"x": 1, "y": 2}}
    d2 = {"a": 1, "b": {"x": 2, "y": 2}}
    result = Comparer.compare(d1, d2)
    assert not result

    result = Comparer.diff(d1, d2)
    assert not result
...
def test_html_diff_dict_fail():
    td = copy.deepcopy(TEST_DICT)
    td["l"] = []
    result = Comparer.html_diff(td, SORTED_DICT)
